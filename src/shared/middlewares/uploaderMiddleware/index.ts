    ...
    ...(customFilter
      ? {
          fileFilter: (req, file, callback) => {
            ...
            if (!ext) {
              return callback(
                new Error('The file extension could not be identified.')
              );
            }
            ...
            function haveExt(data?: string | string[], defaultValue = true) {
              if (data) {
                ...
                const thisIndex = thisData.findIndex(
            ...
            const allowed =
              haveExt(allowExtensions) && !haveExt(blockExtensions, false);
            ...
            if (!allowed) {
              ...
              if (!images) {
                return callback(
                  new Error(`File extension must be equal to ${imagesExt}.`)
                );
              }
            }
